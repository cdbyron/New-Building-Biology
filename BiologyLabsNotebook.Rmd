...
This chunk will install packages the tidyverse package. Also there is an object creation for a dataframe read as the original csv spreadhseet file.
```{r}
library(tidyverse)
...
equipsort <- sort(Equipment.Name)
roomsort <- sort(Room)
EquipInven %>% table(Equipment.Name, Room)

with(d.col, table(Hair, Eye))
```
